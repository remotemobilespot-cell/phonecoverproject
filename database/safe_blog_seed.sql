-- Safe blog posts seed that handles existing data
-- This will update existing posts or insert new ones

-- Clear existing blog data first (optional - uncomment if you want fresh data)
-- DELETE FROM blog_post_tags;
-- DELETE FROM blog_posts;
-- DELETE FROM blog_analytics;

-- Reset sequences (optional - uncomment if cleared above)
...
UPDATE blog_posts SET 
    views = CASE WHEN views = 0 THEN FLOOR(RANDOM() * 2000 + 500) ELSE views END,
    likes = CASE WHEN likes = 0 THEN FLOOR(RANDOM() * 150 + 25) ELSE likes END
WHERE published = true;

-- Verify all posts were created/updated
SELECT 'Blog posts updated successfully:' as status;
SELECT id, title, category, featured, views, likes, created_at, updated_at
FROM blog_posts 
