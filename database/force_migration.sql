-- FORCE DELETE EXISTING TABLES WITH ALL CONSTRAINTS
-- This will handle any foreign key constraints or dependencies
...
DECLARE
    r RECORD;
BEGIN
    -- Drop all foreign key constraints that reference orders or store_locations
    FOR r IN (
        ...
        AND constraint_name IN (
            ...
            WHERE referenced_table_name IN ('orders', 'store_locations')
        )
    ) LOOP
...
-- Now drop the tables with CASCADE to remove all dependencies
DROP TABLE IF EXISTS orders CASCADE;
DROP TABLE IF EXISTS store_locations CASCADE;

-- Also drop any related sequences, indexes, or views that might exist
DROP SEQUENCE IF EXISTS orders_id_seq CASCADE;
DROP SEQUENCE IF EXISTS store_locations_id_seq CASCADE;

-- Drop any existing indexes
DROP INDEX IF EXISTS idx_orders_email;
DROP INDEX IF EXISTS idx_orders_fulfillment;
DROP INDEX IF EXISTS idx_orders_pickup_store;
DROP INDEX IF EXISTS idx_orders_created_at;
DROP INDEX IF EXISTS idx_orders_payment_status;

-- Recreate store_locations table (clean structure)
...
SELECT 'store_locations created with ' || COUNT(*) || ' records' as store_status FROM store_locations;
SELECT 'orders table created and ready' as orders_status;